...
Stage 4: Boss di chuyển thông minh, bắn spread + homing.
"""
...
STAGE_CONFIGS = {
    ...
    4: {
        ...
        "move_speed": 3.5,
        "shoot": True,
        "shoot_interval_frames": 45,   # 0.75 giây tại 60fps
        "bullet_speed": 8.0,
        "pattern": "spread",           # spread hoặc homing
    },
}


def get_stage_config(stage: int) -> dict:
    """Trả về config cho stage, clamp về [1, 4]."""
...
def _spawn_spread(boss_cx: float, boss_cy: float, target_cx: float, target_cy: float, speed: float) -> list:
    """Bắn 5 viên trải đều ±30° quanh hướng Agent."""
    dx = target_cx - boss_cx
    dy = target_cy - boss_cy
...
def _spawn_homing(boss_cx: float, boss_cy: float, target_cx: float, target_cy: float, speed: float) -> list:
    """Bắn 2 viên: 1 straight + 1 homing (đánh dấu để BulletManager xử lý)."""
    dx = target_cx - boss_cx
    dy = target_cy - boss_cy
    dist = math.hypot(dx, dy) or 1.0
    return [
        {"x": boss_cx, "y": boss_cy, "vx": dx / dist * speed, "vy": dy / dist * speed, "homing": False},
        {"x": boss_cx, "y": boss_cy, "vx": dx / dist * speed * 0.6, "vy": dy / dist * speed * 0.6, "homing": True},
    ]
...
def spawn_boss_bullets(pattern: str, boss_cx: float, boss_cy: float,
    ...
    if pattern == "spread":
        # Stage 4: xen kẽ spread và homing ngẫu nhiên
        if random.random() < 0.5:
            return _spawn_spread(boss_cx, boss_cy, target_cx, target_cy, speed)
        return _spawn_homing(boss_cx, boss_cy, target_cx, target_cy, speed)
    return _spawn_straight(boss_cx, boss_cy, target_cx, target_cy, speed)
